// ─────────────────────────────────────────────────────────────
// Profile Bot — creates profiles on high-authority sites (about.me,
// Gravatar, forums, community platforms) with the client's link in
// the website/bio field. Generate short bio → signup → fill profile
// + website field → save. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function runFlow(page, client, site){
  let bioText = client.description;
  try{ const b = await generate('bio', client); bioText = b.bio || b.body || bioText; }catch{}

  await page.goto(site.signupUrl, { waitUntil:'networkidle2', timeout:45000 });
  await browser.humanDelay();
  await smartFill(page, client, { description: bioText });
  ...
  // the key part: make sure the website/link field gets the client URL
  if(site.submitUrl){ await page.goto(site.submitUrl,{waitUntil:'networkidle2',timeout:45000}).catch(()=>{}); await browser.humanDelay(); }
  const urlSel='input[name*="url"],input[name*="website"],input[name*="link"],input[type="url"]';
  if(await page.$(urlSel)){ await browser.humanType(page, urlSel, client.website); }
  const bioSel='textarea[name*="bio"],textarea[name*="about"],textarea';
  if(await page.$(bioSel)){ await page.click(bioSel).catch(()=>{}); await page.keyboard.type(`${bioText||client.name} ${client.website}`,{delay:8}); }

  const shot = await browser.screenshot(page, `profile-${site.name}`);
  const btn = await page.$('button[type=submit],button[name*="save"],input[type=submit]');
  if(btn){ await btn.click().catch(()=>{}); await browser.sleep(2500); }

  return { ok:true, liveUrl:'', screenshot:shot, note:`profile created on ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} profile task(s)`);
...
console.log(`👤 Profile Bot online → ${cfg.API_BASE_URL}`); loop();

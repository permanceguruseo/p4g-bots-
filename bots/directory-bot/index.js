// ─────────────────────────────────────────────────────────────
// Directory Bot — creates business listings (NAP: name/address/phone)
// on business directories. Signup → heavy form fill → category →
// description → CAPTCHA/OTP → submit. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function handleOTP(page, site){
  if(!site.requiresEmailOTP) return;
  const code = await fetchOTP({ fromHint: site.name.toLowerCase() });
  if(!code) throw new Error(`OTP for ${site.name} not received`);
  const sel='input[name*="otp"],input[name*="code"],input[id*="otp"],input[id*="code"]';
...
async function runFlow(page, client, site){
  ...
  // full NAP + business details fill
  const rep = await smartFill(page, client, {
    title: client.name,
    description: client.description || `${client.name} — ${client.category||'business'}`,
  });
  await api.log(`filled ${rep.filled.length} fields on ${site.name}`);

  await handleCaptcha(page, site);
  await handleOTP(page, site);
  await browser.humanDelay();

  const shot = await browser.screenshot(page, `directory-${site.name}`);
  ...
  return { ok:true, liveUrl:'', screenshot:shot, review:!!site.requiresReview,
           note:`listing submitted to ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} directory task(s)`);
...
console.log(`🗂️ Directory Bot online → ${cfg.API_BASE_URL}`); loop();

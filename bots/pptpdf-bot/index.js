// ─────────────────────────────────────────────────────────────
// PPT/PDF Bot — uploads a document to doc-sharing sites (SlideShare,
// Issuu, Scribd style) with the client's link in the description.
// If the client hasn't supplied a file, it generates a simple branded
// PDF first. Uses the shared file-upload handler. Standalone.
// ─────────────────────────────────────────────────────────────
...
const { makePdf } = require('./docGenerator');
...
async function runFlow(page, client, site){
  // use client-provided file, else make a simple branded PDF
  let file = client.docPath;
  if(!file){
    const title = `${client.name} — ${client.category||'Overview'}`;
    const body = (client.description||client.name) + `\n\nLearn more: ${client.website}`;
    file = makePdf(title, body);
    await api.log(`generated PDF for ${client.name}`);
  }
  ...
  // attach the document
  await uploadFile(page, file, { triggerSelector: site.uploadTrigger, dropSelector: site.dropZone });
  await browser.sleep(4000);

  // title + description with link
  const titleSel='input[name*="title"],input[id*="title"]';
  const descSel='textarea[name*="desc"],textarea[id*="desc"],textarea';
  if(await page.$(titleSel)) await browser.humanType(page, titleSel, `${client.name} — ${client.category||''}`);
  if(await page.$(descSel)){ await page.click(descSel).catch(()=>{}); await page.keyboard.type(`${client.description||client.name}\n${client.website}`,{delay:8}); }

  const shot = await browser.screenshot(page, `pptpdf-${site.name}`);
  const btn = await page.$('button[type=submit],button[aria-label*="ublish"],input[type=submit]');
  if(btn){ await btn.click().catch(()=>{}); await browser.sleep(3000); }

  return { ok:true, liveUrl:'', screenshot:shot, note:`document uploaded to ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} pptpdf task(s)`);
...
console.log(`📄 PPT-PDF Bot online → ${cfg.API_BASE_URL}`); loop();

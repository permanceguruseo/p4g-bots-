// ─────────────────────────────────────────────────────────────
// Microblog Bot — posts a short branded update with a link on
// micro-blogging platforms. Generate short post → signup → place
// post + link → submit. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function runFlow(page, client, site){
  const post = await generate('microblog', client);
  const text = `${post.body}\n\n${client.website}`;
  await api.log(`generated micro-post for ${client.name}`);
  ...
  const box = 'textarea,[contenteditable="true"],[role="textbox"]';
  if(await page.$(box)){ await page.click(box).catch(()=>{}); await page.keyboard.type(text,{delay:10}); }
  else throw new Error('no post editor found');

  const shot = await browser.screenshot(page, `microblog-${site.name}`);
  const btn = await page.$('button[type=submit],button[aria-label*="ost"],input[type=submit]');
  if(btn){ await btn.click().catch(()=>{}); await browser.sleep(2500); }

  return { ok:true, liveUrl:'', screenshot:shot, note:`micro-post placed on ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} microblog task(s)`);
...
console.log(`💬 Microblog Bot online → ${cfg.API_BASE_URL}`); loop();

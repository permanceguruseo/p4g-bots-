// ─────────────────────────────────────────────────────────────
// Web 2.0 Bot — publishes a blog post on Web 2.0 platforms
// (WordPress.com / Blogger / Weebly style). Generate post → signup
// → new post → title + body + link in bio → publish. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function runFlow(page, client, site){
  const post = await generate('web2', client);
  await api.log(`generated web2 post "${(post.title||'').slice(0,40)}" for ${client.name}`);
  ...
  if(await page.$(titleSel)) await browser.humanType(page, titleSel, post.title||client.name);
  if(await page.$(bodySel)){ await page.click(bodySel).catch(()=>{}); await page.keyboard.type(post.body+'\n\n'+post.bio,{delay:8}); }
  else throw new Error('no post editor found');

  const shot = await browser.screenshot(page, `web2-${site.name}`);
  ...
  return { ok:true, liveUrl:'', screenshot:shot, note:`web2 post placed on ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} web2 task(s)`);
...
console.log(`🌐 Web2 Bot online → ${cfg.API_BASE_URL}`); loop();

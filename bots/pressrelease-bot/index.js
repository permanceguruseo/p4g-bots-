// ─────────────────────────────────────────────────────────────
// Press Release Bot — writes a formal press release and submits it
// to PR distribution sites (OpenPR / PRLog / PR-inside style).
// Generate PR → signup → title + body + link → CAPTCHA/OTP → submit.
// Most PR sites review before publishing. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function runFlow(page, client, site){
  const pr = await generate('pressrelease', client);
  await api.log(`drafted press release for ${client.name}`);
  ...
  const titleSel='input[name*="title"],input[name*="headline"],input[id*="title"]';
  const bodySel='textarea,[contenteditable="true"]';
  if(await page.$(titleSel)) await browser.humanType(page, titleSel, pr.title||`${client.name} News`);
  if(await page.$(bodySel)){ await page.click(bodySel).catch(()=>{}); await page.keyboard.type(pr.body+'\n\n'+pr.bio,{delay:6}); }
  else throw new Error('no PR body field found');

  const shot = await browser.screenshot(page, `pressrelease-${site.name}`);
  const btn = await page.$('button[type=submit],input[type=submit]');
  if(btn){ await btn.click().catch(()=>{}); await browser.sleep(2500); }

  return { ok:true, liveUrl:'', screenshot:shot, review:true, note:`PR submitted to ${site.name} (awaiting review)` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} pressrelease task(s)`);
...
console.log(`📰 Press Release Bot online → ${cfg.API_BASE_URL}`); loop();

// ─────────────────────────────────────────────────────────────
// Classified Bot — posts a classified ad (Locanto / Storeboard /
// ClassifiedAds style) with the client's link. Signup → category →
// title + description + location → CAPTCHA/OTP → post. Standalone.
// ─────────────────────────────────────────────────────────────
...
async function runFlow(page, client, site){
  ...
  const title = `${client.name} — ${client.category||'Services'}`;
  const desc = `${client.description || client.name}\n\nVisit: ${client.website}`;

  // classifieds use title/description + contact + location
  await smartFill(page, client, { title, description: desc });

  // category dropdown, if present, pick the first sensible option
  const cat = await page.$('select[name*="cat"],select[id*="cat"]');
  if(cat){ await page.evaluate(el=>{ if(el.options.length>1) el.selectedIndex=1; el.dispatchEvent(new Event('change',{bubbles:true})); }, cat); }
  ...
  const shot = await browser.screenshot(page, `classified-${site.name}`);
  const btn = await page.$('button[type=submit],button[name*="post"],input[type=submit]');
  if(btn){ await btn.click().catch(()=>{}); await browser.sleep(2500); }

  return { ok:true, liveUrl:'', screenshot:shot, review:!!site.requiresReview,
           note:`classified posted on ${site.name}` };
}
...
async function loop(){
  ...
  try{ const tasks = await api.getMyTasks();
    if(tasks.length){ await api.log(`picked up ${tasks.length} classified task(s)`);
...
console.log(`📢 Classified Bot online → ${cfg.API_BASE_URL}`); loop();
